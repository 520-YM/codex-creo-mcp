...
static int is_resumable_feature_type(ProFeattype type)
{
...
static ProError verify_active_parents(ProSolid solid, ProFeature *feature)
{
    int *parents = NULL;
    int parent_count = 0;
    int index;
    ...
    status = ProFeatureParentsGet(feature, &parents, &parent_count);
    ...
    for (index = 0; index < parent_count; ++index)
    {
        ...
        ProFeatStatus parent_status = PRO_FEAT_INVALID;

        status = ProFeatureInit(solid, parents[index], &parent_feature);
        if (status != PRO_TK_NO_ERROR)
            break;
        status = ProFeatureStatusGet(&parent_feature, &parent_status);
        if (status != PRO_TK_NO_ERROR)
            break;
        if (parent_status != PRO_FEAT_ACTIVE)
        {
            status = PRO_TK_SUPP_PARENTS;
            break;
        }
    }
...
int wmain(int argc, wchar_t **argv)
{
    ...
    {
        fwprintf(stderr,
            L"Usage: creo_feature_resume_bridge <result.json> <source_file> "
            L"<expected_model> <copy_name> <output_dir> <feature_id> "
            L"<expected_type_code>\n");
        return 2;
    }
    ...
    feature_id_long = wcstol(argv[6], &parse_end, 10);
    if (*argv[6] == L'\0' || *parse_end != L'\0' ||
        feature_id_long <= 0 || feature_id_long > INT_MAX)
    {
        exit_code = write_error(out, "feature_id_input", PRO_TK_BAD_INPUTS);
        goto done;
    }
    expected_type_long = wcstol(argv[7], &parse_end, 10);
    if (*argv[7] == L'\0' || *parse_end != L'\0' ||
        expected_type_long <= 0 || expected_type_long > INT_MAX)
    {
    ...
    if (GetFileAttributesW(source_path) == INVALID_FILE_ATTRIBUTES)
    {
        exit_code = write_error(out, "source_file", PRO_TK_E_NOT_FOUND);
        goto done;
    }
    ...
    if (status != PRO_TK_NO_ERROR)
    {
        exit_code = write_error(out, "load_source_file", status);
        goto cleanup;
    }
    source_loaded = 1;
    ...
    if (!is_resumable_feature_type(source_feature_type))
    {
        exit_code = write_error(out, "resumable_type_guard", PRO_TK_NO_PERMISSION);
        goto cleanup;
    }
    status = ProFeatureStatusGet(&source_feature, &source_feature_status);
    if (status != PRO_TK_NO_ERROR || source_feature_status != PRO_FEAT_SUPPRESSED)
    {
    ...
    status = verify_active_parents((ProSolid)source_model, &source_feature);
    if (status != PRO_TK_NO_ERROR)
    {
        exit_code = write_error(out, "active_parents_guard", status);
        goto cleanup;
    }
    ...
    {
        exit_code = write_error(out, "copy_suppression_guard", PRO_TK_GENERAL_ERROR);
        goto cleanup;
    }
    ...
    if (status != PRO_TK_NO_ERROR)
    {
        exit_code = write_error(out, "feature_resume", status);
        goto cleanup;
    }
    status = ProFeatureInit((ProSolid)copy_model, feature_id, &copy_feature);
    if (status != PRO_TK_NO_ERROR ||
        ProFeatureStatusGet(&copy_feature, &copy_feature_status) != PRO_TK_NO_ERROR ||
        copy_feature_status != PRO_FEAT_ACTIVE)
    {
        exit_code = write_error(out, "resume_readback", PRO_TK_GENERAL_ERROR);
        goto cleanup;
    }
    ...
    fputs("{\"ok\":true,\"safe_copy_only\":true,\"source_file\":", out);
    write_wide_json_string(out, source_path);
    fputs(",\"source_model\":", out);
