...
typedef struct parameter_update_record
{
    ProName name;
    ProLine old_value;
    ProLine new_value;
} ParameterUpdateRecord;
...
int wmain(int argc, wchar_t **argv)
{
    ...
    ProPath saved_path;
    ParameterUpdateRecord updates[MAX_UPDATES];
    int update_count;
    int i;
    int j;
    int connected = 0;
    ...
    {
        fwprintf(stderr,
            L"Usage: creo_multi_write_bridge <result.json> <expected_model> "
            L"<copy_name> <output_dir> <parameter> <value> [<parameter> <value> ...]\n");
        return 2;
    }

    update_count = (argc - 5) / 2;
    if (update_count < 1 || update_count > MAX_UPDATES)
    {
        fwprintf(stderr, L"The update count must be between 1 and %d.\n", MAX_UPDATES);
        return 2;
    }
    ...
    {
        const wchar_t *name_arg = argv[5 + i * 2];
        const wchar_t *value_arg = argv[6 + i * 2];

        if (!is_allowed_parameter(name_arg))
        {
            exit_code = write_error(out, "parameter_allowlist", PRO_TK_NO_PERMISSION);
            goto done;
        }
        if (wcslen(name_arg) >= PRO_NAME_SIZE || wcslen(value_arg) >= PRO_LINE_SIZE)
        {
            exit_code = write_error(out, "parameter_input_length", PRO_TK_BAD_INPUTS);
            goto done;
        }

        wcsncpy_s(updates[i].name,
            sizeof(updates[i].name) / sizeof(updates[i].name[0]),
            name_arg, _TRUNCATE);
        wcsncpy_s(updates[i].new_value,
            sizeof(updates[i].new_value) / sizeof(updates[i].new_value[0]),
            value_arg, _TRUNCATE);

        for (j = 0; j < i; ++j)
        {
            if (_wcsicmp(updates[i].name, updates[j].name) == 0)
            {
                exit_code = write_error(out, "duplicate_parameter", PRO_TK_BAD_INPUTS);
                goto done;
            }
        }
    }
    ...
    status = ProMdlToModelitem(source_model, &source_item);
    if (status != PRO_TK_NO_ERROR)
    {
        exit_code = write_error(out, "source_model_item", status);
        goto cleanup;
    }

    for (i = 0; i < update_count; ++i)
    {
        status = ProParameterInit(&source_item, updates[i].name, &parameter);
        if (status != PRO_TK_NO_ERROR)
        {
            exit_code = write_error(out, "source_parameter_init", status);
            goto cleanup;
        }
        ...
        {
            exit_code = write_error(out, "source_parameter_type_guard",
                status == PRO_TK_NO_ERROR ? PRO_TK_INVALID_TYPE : status);
            goto cleanup;
        }
        wcsncpy_s(updates[i].old_value,
            sizeof(updates[i].old_value) / sizeof(updates[i].old_value[0]),
            value.value.s_val, _TRUNCATE);
    }
    ...
    for (i = 0; i < update_count; ++i)
    {
        status = ProParameterInit(&copy_item, updates[i].name, &parameter);
        if (status != PRO_TK_NO_ERROR)
        {
            exit_code = write_error(out, "copy_parameter_init", status);
            goto cleanup;
        }
        status = ProParamvalueSet(&new_value, updates[i].new_value, PRO_PARAM_STRING);
        if (status != PRO_TK_NO_ERROR)
        {
        ...
        if (status != PRO_TK_NO_ERROR ||
            ...
            wcscmp(verified_value.value.s_val, updates[i].new_value) != 0)
        {
    ...
    for (i = 0; i < update_count; ++i)
    {
        if (i > 0)
            fputc(',', out);
        fputs("{\"parameter\":", out);
        write_wide_json_string(out, updates[i].name);
        fputs(",\"old_value\":", out);
        write_wide_json_string(out, updates[i].old_value);
        fputs(",\"new_value\":", out);
        write_wide_json_string(out, updates[i].new_value);
        fputc('}', out);
    }
    fprintf(out, "],\"regenerate_status\":%d,\"saved_file\":", regenerate_status);
